...
class Clrc663 {
	public:
	Clrc663(uint8_t address = 0x28);
	
	int init(uint8_t address = 0x28);
  void pcaselect(uint8_t paddr,uint8_t channel) ;
  void pcaclose(uint8_t paddr, uint8_t channel);
...
/*!
  ...
  \param [in] reg Specifies at which register reading starts.
  \param [in] values An array of the values to read from the register starting from `reg`.
              The first value (`values[0]`) gets readen from `reg`, the second (`values[1]`) to `reg+1`, and so on.
  \param [in] len The number of register to read.
  \return the number or byte readed or -1 in case of error
 */
 int read_regs(uint8_t reg,  uint8_t* values, uint8_t len);
...
/*!
  ...
  \return the number of byte written , -1 in case of error
 */
 int  write_regs(uint8_t reg, const uint8_t* values, uint8_t len);
 ...
 */
void cmd_read_E2(uint16_t address, uint16_t length);
 ...
 */
void cmd_load_key_E2(uint8_t key_nr);
 ...
 */
void cmd_load_key(const uint8_t* key);
...
/*!
  ...
  This function is usually preceded by either the `cmd_load_key_E2()` or `cmd_load_key()` functions.
 ...
 */
void cmd_auth(uint8_t key_type, uint8_t block_address, const uint8_t* card_uid);
...
void print_buffer_hex(const uint8_t* data, uint16_t len);
...
/*!
  ...
  This basically calls cmd_read_E2() and then transfers the FIFO with read_fifo(). This is useful for
 ...
 */
uint8_t transfer_E2_page(uint8_t* dest, uint8_t page);
...
*/
void AN11145_start_IQ_measurement();
...
/*! \brief Stop IQ Measurement.
    ...
    \code
      uint8_t I_value = read_reg(MFRC630_REG_LPCD_I_RESULT) & 0x3F
      uint8_t Q_value = read_reg(MFRC630_REG_LPCD_Q_RESULT) & 0x3F
    \endcode
*/
void AN11145_stop_IQ_measurement();
...
/*! \defgroup iso14443a ISO14443A
    ...
    order (`iso14443a_WUPA_REQA()`, `iso14443a_select()`, and then a function from @ref mifare) the
...
/*! \brief Perform a MIFARE authentication procedure.
    ...
    internally it calls `cmd_auth()` which is described above, the result of the
    ...
    \code
         void cmd_load_key_E2(uint8_t key_nr);
         void cmd_load_key(uint8_t* key);
    \endcode
    Once authenticated, the authentication MUST be stopped manually by calling
    the `MF_deauth()` function or otherwires disabling the Crypto1 ON bit
...
*/
uint8_t MF_auth(const uint8_t* uid, uint8_t key_type, uint8_t block);
...
*/
uint8_t MF_read_block(uint8_t block_address, uint8_t* dest);
...
*/
uint8_t MF_write_block(uint8_t block_address, const uint8_t* source);
...
/*! \brief An example to read the first four blocks.
  ...
  Load the correct register settings with:
    \code
      AN1102_recommended_registers(MFRC630_PROTO_ISO14443A_106_MILLER_MANCHESTER);
    \endcode
  ...
  Then attempt to authenticate block 0 using this FFkey as KEY_A type:
    \code
      MF_auth(uid, MFRC630_MF_AUTH_KEY_A, 0)
    \endcode
  Finally, if that succeeds, we may use:
    \code
      len = MF_read_block(0, readbuf)
    \endcode
